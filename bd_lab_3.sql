...
SELECT code, price, price*40.31 AS UAН, price*0.91 AS EUR FROM `books`;

/*15. Вивести значення наступних колонок: код, ціна, ціна в грн. без копійок, ціна без копійок округлена*/
SELECT code, price, price * 40.31 AS UAН, TRUNCATE(price*40.31,0) AS UAН1, ROUND(price*40.31,0) AS UAН2 FROM `books`; 
